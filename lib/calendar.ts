...
class VietnameseHelper extends HelperBase {
  id = 'vietnamese' as const;
  calendarType = 'lunisolar' as const;

  // Timezone offset for Vietnam (UTC+7)
  private readonly VIETNAM_TZ_OFFSET = 7.0;
  ...
  maxLengthOfMonthCodeInAnyYear(monthCode: string) {
    return 30; // All months can have up to 30 days
  }

  // Convert Gregorian date to Julian Day Number
  private jdFromDate(year: number, month: number, day: number): number {
  ...
  private jdToDate(jd: number): { year: number; month: number; day: number } {
    let a, b, c, d, e, m;
  ...
  private getSunLongitude(jdn: number): number {
    let T = (jdn - 2451545.5 - this.VIETNAM_TZ_OFFSET / 24) / 36525;
    let T2 = T * T;
    let dr = Math.PI / 180;
  ...
  private isoToVietnameseLunar(isoDate: ISODate): { year: number; month: number; day: number; leap: boolean } {
    const { year, month, day } = isoDate;
    const dayNumber = this.jdFromDate(year, month, day);
  ...
  private vietnameseLunarToIso(lunarYear: number, lunarMonth: number, lunarDay: number, lunarLeap: boolean): ISODate {
    let a11, b11;

    if (lunarMonth < 11) {
      a11 = this.getLunarMonth11(lunarYear - 1);
      b11 = this.getLunarMonth11(lunarYear);
    } else {
      a11 = this.getLunarMonth11(lunarYear);
      b11 = this.getLunarMonth11(lunarYear + 1);
    }
    ...
    return this.jdToDate(isoJd);
  }

  override getFormatter(): never {
  ...
  getMonthList(calendarYear: number, cache: OneObjectCache): ChineseMonthInfo {
    ...
    const a11_prev = this.getLunarMonth11(calendarYear - 1);
    const a11_curr = this.getLunarMonth11(calendarYear);
    const a11_next = this.getLunarMonth11(calendarYear + 1);
    ...
    if (a11_curr - a11_prev > 365) {
      ...
      for (let i = 0; i < 13; i++) {
        const monthStart = this.getNewMoonDay(k + 2 + i);
        const nextMonthStart = this.getNewMoonDay(k + 2 + i + 1);

        const sunLong1 = this.getSunLongitude(monthStart);
        const sunLong2 = this.getSunLongitude(nextMonthStart);
      ...
      for (let i = 0; i < 4; i++) {
        const monthStart = this.getNewMoonDay(k + i);
        const nextMonthStart = this.getNewMoonDay(k + i + 1);
  ...
  estimateIsoDate(calendarDate: CalendarYMD): ISODate {
    const { year, month } = calendarDate;
    return { year, month: month >= 12 ? 12 : month + 1, day: 1 };
  }
  ...
  override adjustCalendarDate(
    ...
    this.validateCalendarDate(calendarDate);
    let { year, month, monthCode, day } = calendarDate;
    ...
    if (overflow === 'reject') {
      ES.RejectToRange(day, 1, 30);
    } else {
      day = ES.ConstrainToRange(day, 1, 30);
    }

    return { year, month, monthCode, day };
  }

  override calendarToIsoDate(
    dateParam: CalendarDateFields,
    overflow: Overflow = 'constrain',
    cache: OneObjectCache
  ): ISODate {
    const date = this.adjustCalendarDate(dateParam, cache, overflow, false);
    const { year, month, day, monthCode } = date;

    const key = JSON.stringify({ func: 'calendarToIsoDate', year, month, day, monthCode, overflow, id: this.id });
    const cached = cache.get(key);
    if (cached) return cached;

    const isoDate = this.vietnameseLunarToIso(year, +monthCode.substring(1, 3), day, monthCode.endsWith('L'));

    if (isoDate.year === 0) {
      throw new RangeError(`Ngày âm lịch không hợp lệ: ${year}/${month}/${day}`);
    }

    cache.set(key, isoDate);
    return isoDate;
  }

  override monthDaySearchStartYear(monthCode: string, day: number) {
    void day;
    const month = +monthCode.replace(/^M0?|L$/g, '');
    return 1900 + (month % 5) * 10;
  }
}
